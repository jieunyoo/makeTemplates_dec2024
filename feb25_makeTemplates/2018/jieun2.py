...
import pyarrow
import yaml
from systematicsFail import get_systematic_dict, sigs
    ...
    for year in years:  # e.g. 2018, 2017, 2016APV, 2016
        for ch in channels:  # e.g. mu, ele
            ...
            for sample in os.listdir(samples_dir[year]):
                ...
                except pyarrow.lib.ArrowInvalid:  # empty parquet because no event failed selection
                    continue
...
#********************************************************************************************************
                ...
                for region, region_sel in regions_sel.items():  # e.g. fail, fail, top control region, etc.
...
#*********************************************************************************************************************************
                ...
                #************
                    ...
                    hists.fill( Sample=sample_to_use, Systematic="fail_nominal", Region=region, mass_observable=df["fj_mass"], weight=nominal,)
...
#nominal has event weight, but event weight already has pileup weight in it since it is a product of various event weights, so i guess that is why farouk only has xsec weight.
  ...
  #*****************************************************************************************************************
                    ...
                    hists.fill(Sample=sample_to_use, Systematic=f"fail_weight_pdf_acceptance_{sample_to_use}_up", Region=region, mass_observable=df["fj_mass"], weight=shape_up, )
                    hists.fill( Sample=sample_to_use, Systematic=f"fail_weight_pdf_acceptance_{sample_to_use}_down", Region=region, mass_observable=df["fj_mass"], weight=shape_down,)
                    ...
                    hists.fill(Sample=sample_to_use, Systematic=f"fail_weight_qcd_scale_{sample_to_use}_up", Region=region, mass_observable=df["fj_mass"],  weight=shape_up,)
                    hists.fill( Sample=sample_to_use, Systematic=f"fail_weight_qcd_scale_{sample_to_use}_down", Region=region, mass_observable=df["fj_mass"], weight=shape_down, )
...
#*****************************************************************************************                   
                    ...
                    hists.fill( Sample=sample_to_use, Systematic="fail_top_reweighting_up", Region=region, mass_observable=df["fj_mass"], weight=shape_up,)
                    hists.fill( Sample=sample_to_use, Systematic="fail_top_reweighting_down", Region=region, mass_observable=df["fj_mass"], weight=shape_down,)

           
#JMR/JMS**********************************************************************************
                    for syst, (yrs, smpls, var) in SYST_DICT["JEC_systs_mass"].items():
                        #if (sample_to_use in smpls) and (year in yrs) and (ch in var): 
                        if (sample_to_use in ["ZH", "WH", "ggF", "VBF", "ttH", "TTbar", "Diboson", "SingleTop"]) and (year in yrs) and (ch in var):  
...
'fail_weight_Vtagger_up',
'fail_weight_Vtagger_down',
'fail_weight_JMS_up',
'fail_weight_JMS_down',
'fail_weight_JMR_up',
'fail_weight_JMR_down',
