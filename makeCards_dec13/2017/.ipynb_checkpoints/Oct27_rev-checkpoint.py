...
def smass(sName):
    if sName in ['ggF','VBF','WH','ZH','ttH']:
        _mass = 125.
    elif sName in ['WJetsLNu','EWKW','TTbar','SingleTop','EWKvjets', 'WZQQ', 'Diboson']:
        _mass = 80.379
...
def get_templ(f, region, sample, syst=None, muon=False, nowarn=False):
    ...
    if np.any(h_vals < 0):
        print("Sample negative bins. They will be set to 0.")
        _invalid = h_vals < 0
        h_vals[_invalid] = 0
        h_variances[_invalid] = 0
    if np.any(~np.isfinite(h_vals)):
...
def dummy_rhalphabet( scale_syst=True, smear_syst=True,  systs=True,muonCR=False,year='2017',  opts=None):
    ...
    sys_shape_dict['weight_pileup_id'] = rl.NuisanceParameter('CMS_pileup_id', 'shape')
    sys_shape_dict["weight_L1Prefiring"] = rl.NuisanceParameter('CMS_l1_ecal_prefiring_{}'.format(year),'shape')


    #JEC related
    sys_shape_dict['UES'] = rl.NuisanceParameter('unclustered_Energy', 'shape')
    sys_shape_dict['JES_FlavorQCD'] = rl.NuisanceParameter('CMS_scale_j_FlavQCD', 'shape')
    sys_shape_dict['JES_RelativeBal'] = rl.NuisanceParameter('CMS_scale_j_RelBal', 'shape')
    sys_shape_dict['JES_HF'] = rl.NuisanceParameter('CMS_scale_j_HF', 'shape')
    sys_shape_dict['JES_BBEC1'] = rl.NuisanceParameter('CMS_scale_j_BBEC1', 'shape')
    sys_shape_dict['JES_EC2'] = rl.NuisanceParameter('CMS_scale_j_EC2', 'shape')
    sys_shape_dict['JES_Absolute'] = rl.NuisanceParameter('CMS_scale_j_Abs', 'shape')

    sys_shape_dict["JER_year"] = rl.NuisanceParameter('CMS_res_j_{}'.format(year),'shape')
    sys_shape_dict["JES_BBEC1_year"] = rl.NuisanceParameter('CMS_scale_j_BBEC1_{}'.format(year),'shape')
    sys_shape_dict["JES_RelativeSample_year"] = rl.NuisanceParameter('CMS_scale_j_RelSample_{}'.format(year),'shape')
    sys_shape_dict["JES_EC2_year"] = rl.NuisanceParameter('CMS_scale_j_EC2_{}'.format(year),'shape')
    sys_shape_dict["JES_HF_year"] = rl.NuisanceParameter('CMS_scale_j_HF_{}'.format(year),'shape')
    sys_shape_dict["JES_Absolute_year"] = rl.NuisanceParameter('CMS_scale_j_Abs_{}'.format(year),'shape')
    ...
    for region in ['pass', 'fail']:
        ...
        include_samples = [
                'WH', 'ZH', 'VBF', 'ggF', 'ttH',
                'WJetsLNu',  'TTbar', 'SingleTop','DYJets', 'Diboson',
           # 'WZQQ',# 'EWKvjets',
            ]
    ...
    #**********************************************************************************
            ...
            if not systs:  # Need at least one
                ...
                if sName in ['WH', 'ZH', 'ggF', 'VBF', 'ttH']: sample.setParamEffect(sys_tagger,1.244,0.907 )
               ...
               "weight_d1K_NLO", "weight_d2K_NLO", "weight_d3K_NLO",
                ...
                sys_names = [ "weight_pileup", "weight_pileup_id",  'weight_L1Prefiring',
               'weight_pdf_acceptance_WH','weight_qcd_scale_WH',  'weight_pdf_acceptance_ZH','weight_qcd_scale_ZH',
                ...
                'weight_pdf_acceptance_ttH','weight_qcd_scale_ttH',  'weight_pdf_acceptance_TTbar','weight_qcd_scale_TTbar',
                'weight_pdf_acceptance_SingleTop','weight_qcd_scale_SingleTop', 'weight_pdf_acceptance_WJetsLNu', 'weight_qcd_scale_WJetsLNu',
                'top_reweighting',
                ...
                #SCALE FACTOR*****************************
                if sName not in ["qcd", 'WJetsLNu', 'DYJets', 'ZH']:
   ...
   #     if region == 'pass':
   #         print('pass fake region')
   #         if sNameFake == 'fake': 
   #             fakeSample.setParamEffect(fake_SF_uncert,1.3,None)
...
#for topCR
    failCh = model['SR1fail{}'.format(year)]
    passCh = model['SR1pass{}'.format(year)]
...
#******************************************************
ROOTFILE = '/home/jieun/VH-makeCards/2017/2017.root'
import os
thisdir = os.getcwd()
