...
def one_bin2(f, region, sample, syst=None, muon=False, nowarn=False):
...
def smass(sName):
    if sName in ['ggF','VBF','WH','ZH','ttH']:
        _mass = 125.
    elif sName in ['WJetsLNu','TTbar','SingleTop','WZQQ', 'EWKvjets', 'Diboson']:
        _mass = 80.379
...
def dummy_rhalphabet( scale_syst=True, smear_syst=True,  systs=True,muonCR=False,year='2017',  opts=None):
 ...
 #general shapes
    ...
    sys_shape_dict["weight_L1Prefiring"] = rl.NuisanceParameter('CMS_l1_ecal_prefiring_{}'.format(year),'lnN')
    ...
    sys_shape_dict['weight_pdf_acceptance_WH'] = rl.NuisanceParameter('PDF_WH_ACCEPT_CMS_HWW_boosted', 'lnN')
    sys_shape_dict['weight_qcd_scale_WH'] = rl.NuisanceParameter('QCDscale_WH_ACCEPT_CMS_HWW_boosted', 'lnN')
    sys_shape_dict['weight_pdf_acceptance_ZH'] = rl.NuisanceParameter('PDF_ZH_ACCEPT_CMS_HWW_boosted', 'lnN')
    sys_shape_dict['weight_qcd_scale_ZH'] = rl.NuisanceParameter('QCDscale_ZH_ACCEPT_CMS_HWW_boosted', 'lnN')
    sys_shape_dict['weight_pdf_acceptance_ggF'] = rl.NuisanceParameter('PDF_ggH_ACCEPT_CMS_HWW_boosted', 'lnN')
    sys_shape_dict['weight_qcd_scale_ggF'] = rl.NuisanceParameter('QCDscale_ggH_ACCEPT_CMS_HWW_boosted', 'lnN')
    sys_shape_dict['weight_pdf_acceptance_VBF'] = rl.NuisanceParameter('PDF_qqH_ACCEPT_CMS_HWW_boosted', 'lnN')
    sys_shape_dict['weight_qcd_scale_VBF'] = rl.NuisanceParameter('QCDscale_qqH_ACCEPT_CMS_HWW_boosted', 'lnN')
    sys_shape_dict['weight_pdf_acceptance_ttH'] = rl.NuisanceParameter('PDF_ttH_ACCEPT_CMS_HWW_boosted', 'lnN')
    sys_shape_dict['weight_qcd_scale_ttH'] = rl.NuisanceParameter('QCDscale_ttH_ACCEPT_CMS_HWW_boosted', 'lnN')
    ...
    sys_shape_dict['weight_pdf_acceptance_SingleTop'] = rl.NuisanceParameter('PDF_singletop_ACCEPT_CMS_HWW_boosted', 'lnN')
    sys_shape_dict['weight_pdf_acceptance_WJetsLNu'] = rl.NuisanceParameter('PDF_wjets_ACCEPT_CMS_HWW_boosted', 'lnN')
    sys_shape_dict['weight_qcd_scale_WJetsLNu'] = rl.NuisanceParameter('QCDscale_wjets_ACCEPT_CMS_HWW_boosted', 'lnN')
    ...
    sys_shape_dict['weight_PSFSR_WJetsLNu'] = rl.NuisanceParameter('ps_fsr_wjets', 'lnN')
    sys_shape_dict['weight_PSFSR_ttH'] = rl.NuisanceParameter('ps_fsr_ttH', 'lnN')
    sys_shape_dict['weight_PSFSR_TTbar'] = rl.NuisanceParameter('ps_fsr_ttbar', 'lnN')
    sys_shape_dict['weight_PSFSR_SingleTop'] = rl.NuisanceParameter('ps_fsr_singletop', 'lnN')
    ...
    #print('f.Get(SingleTop_pass_weight_btagSFbcCorrelated_down', f['SingleTop_pass_weight_btagSFbcCorrelated_down'])
    ...
    #**********************************************************************************
            ...
            if not systs:  # Need at least one
                ...
                sys_names_toLnN = ["weight_btagSFlightCorrelated", 'weight_btagSFbcCorrelated', 'weight_btagSFlight', 'weight_btagSFbc',
               "weight_d1K_NLO", "weight_d2K_NLO", "weight_d3K_NLO",
                ...
                #********************************************************   
                    ...
                    if sys_shape_dict[sys_name].combinePrior == "lnN":
                        nominal = get_templ(f, region, sName,syst='nominal') 
                        syst_up = get_templ(f, region, sName, syst=sys_name + "_up")
                        syst_down = get_templ(f, region, sName, syst=sys_name + "_down")
                        nominal = one_bin(nominal)
                        _up = one_bin(syst_down)
                        _down = one_bin(syst_down)
                    
                        #print('up', _up)
                        #print('up[0]', _up[0])
                        #print('up/nom', _up[0]/nominal[0])
                    
                    
                        if math.isclose(_up[0], _down[0], rel_tol=0.02):  # if up and down are the same
                            arr = np.array([_up[0]/nominal[0],_down[0]/nominal[0]])
                            sample.setParamEffect(sys_shape_dict[sys_name], np.mean(arr))    
                        else:
                            sample.setParamEffect(sys_shape_dict[sys_name], max(_up, _down), min(_up, _down))
                    else:
                ...
                sys_names = [ "weight_pileup", "weight_pileup_id",
                              'weight_L1Prefiring',
       ...
       #     ch.addSample(fakeSample)
        ...
        print('data-obs', data_obs)

        #ch.autoMCStats(     channel_name=f"CMS_HWW_boosted_TopCR_{region}", )
        #ch.autoMCStats(lnN=True)
...
#******************************************************
ROOTFILE = '/home/jieun/VH-makeCards/2017/2017_TopCR.root'
#******************************************************
