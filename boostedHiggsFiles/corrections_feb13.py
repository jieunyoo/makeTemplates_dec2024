...
def get_jmsr(fatjets, num_jets: int, year: str, isData: bool = False,seed: int = 42) -> dict:
  ...
  #  (smearing * np.maximum(jmrValues[mkey][year][i] - 1, 0) + 1) for i in range(3)
#]
            np.random.seed(seed)
            smearing_nom = np.random.normal(0,1,size=mass.shape)

            np.random.seed(seed+1)
            smearing_down = np.random.normal(0,1,size=mass.shape)

            np.random.seed(seed+2)
            smearing_up = np.random.normal(0,1,size=mass.shape)
    ...
    smearing * (jmrValues[mkey][year][i] - 1) + 1 for i in range(3)
            ...
            #smearup = smearing_up
            #smeardown = smearing_down
            #smearnom = smearing_nom
        jmsr_shifted_vars[mkey] = tdict

        #print('dictionary', jmsr_shifted_vars[mkey])
...
def OLDget_jmsr(fatjets, num_jets: int, year: str, isData: bool = False, seed: int = 42) -> Dict:
    """Calculates post JMS/R masses and shifts"""
    jmsr_shifted_vars = {}
...
def getJECVariables_Higgs(fatjetvars, met, pt_shift=None, met_shift=None):
#    """
...
#    """
    ...
    variables[f"rec_H_pt{shift}"] = candidatefj.pt
    return variables
...
#    """
    variables = {}
    #print('fatjetpt', ak.to_list(fatjetvars["fj_pt"][0:200]))
    #print('met', ak.to_list(met)[0:500])
    ...
    if met_shift is not None:
        ...
        if ptlabel != "":
            if ptlabel == "JES_up":
                metvar = met.JES_jes.up
                #print('metvar', ak.to_list(metvar)[0:500])
            elif ptlabel == "JES_down":
                metvar = met.JES_jes.down
            # elif ptlabel == "JER_up":
            #     metvar = met.JER.up
