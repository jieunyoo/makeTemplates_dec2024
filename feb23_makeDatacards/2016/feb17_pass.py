...
#*********************************************************************************
   ...
   # sys_shape_dict['weight_qcd_scale_WJetsLNu'] = rl.NuisanceParameter('QCDscale_wjets_ACCEPT_CMS_HWW_boosted', 'shape')
    ...
    sys_shape_dict['JMR_sig_2016'] = rl.NuisanceParameter('CMS_HWW_boosted_jmr_sig_{}'.format(year), 'shape') 
    sys_shape_dict['JMS_sig_2016'] = rl.NuisanceParameter('CMS_HWW_boosted_jms_sig_{}'.format(year), 'shape') 

    sys_shape_dict['JMR_wjets_2016'] = rl.NuisanceParameter('CMS_HWW_boosted_jmr_wjets_{}'.format(year), 'shape') 
    sys_shape_dict['JMS_wjets_2016'] = rl.NuisanceParameter('CMS_HWW_boosted_jms_wjets_{}'.format(year), 'shape') 

    sys_shape_dict['JMR_ttbar_2016'] = rl.NuisanceParameter('CMS_HWW_boosted_jmr_ttbar_{}'.format(year), 'shape') 
    sys_shape_dict['JMS_ttbar_2016'] = rl.NuisanceParameter('CMS_HWW_boosted_jms_ttbar_{}'.format(year), 'shape') 
              ...
              #   'weight_qcd_scale_WJetsLNu',
                ...
                for sys_name in sys_names:   
                    ...
                    if ("weight_pdf_acceptance_WJetsLNu" in sys_name) and sName not in ['WJetsLNu']:
                        continue
                    if ("JMR_sig_2016" in sys_name) and sName not in ['WH', 'ZH', 'ggF', 'VBF', 'ttH']:      
                        continue
                    if ("JMS_sig_2016" in sys_name) and sName not in ['WH', 'ZH', 'ggF', 'VBF', 'ttH']:          
                        continue    
      
                    if ("JMR_wjets_2016" in sys_name) and sName not in ['WJetsLNu', 'DYJets']:
                        continue
                    if ("JMS_wjets_2016" in sys_name) and sName not in ['WJetsLNu', 'DYJets']:
                        continue    


                    if ("JMR_ttbar_2016" in sys_name) and sName not in ['TTbar', 'SingleTop', 'Diboson', 'EWKvjets']:
                        continue
                    if ("JMS_ttbar_2016" in sys_name) and sName not in ['TTbar', 'SingleTop', 'Diboson', 'EWKvjets']:
                        continue    
                ...
                for sys_name in btaggingList:
                    if sys_shape_dict[sys_name].combinePrior == "lnN":
                        ...
                        #print('syst up', syst_up)
                        #print('syst down', syst_down)
                        #print('eff_up', eff_up)
                        #print('eff down', eff_down)
                        max_variation = max(abs(eff_up), abs(eff_down))
                        ...
                        if math.isclose(eff_up, eff_down, rel_tol=0.02):  # if up and down are the same                           
                            ...
                            #print('setting the max and max variation', max_variation)
                            sample.setParamEffect(sys_shape_dict[sys_name], max_variation, None)
...
#******************************************************
ROOTFILE = '/home/jieun/VH-makeCards2/feb4_2016/2016_pass.root'
import os
thisdir = os.getcwd()
